...
Scenes.prototype.gotoScene = function(sceneID,option){
  if(!this.scrollPossible)return;
  this.scrollPossible = false;
  console.log('a',this.frame.current,this.getScene(sceneID).frame.start);
  if(this.scene.current < sceneID){
    ...
    this.direction = 0;
    if(this.scene.current > 0){
      console.log('a',this.scene.current,sceneID);
      if(this.getScene(sceneID-1).option.loop != 0){
        this.gotoFrame(this.getScene(sceneID-1).option.loop-1,option);  
      }else{
        this.gotoFrame(this.getScene(sceneID).frame.start,option);
      }
    }else{
...
Scenes.prototype.gotoFrame = function(tgFrame,option) {
  var offset  = { current : tgFrame, estimate : tgFrame },
      config  = { duration:1, ease:Power0.easeNone },
      scope   = this;
  $.extend(config,option);
  offset.ease = config.ease;
  if(config.delay)offset.delay = config.delay;

  if(config.duration==0){
    $.extend(this.frame,offset);
    onComplete(config);
  }else{
    offset.onUpdate = stepAnimation;
    offset.onComplete = function(){ 
      onComplete(config);
    };
    offset.onStart = function(){
      $(scope).trigger(scope.EVENT_DELETE_SCROLL);
      if(config.onStart)config.onStart();
    }
    offset.ease = config.ease;
    TweenLite.to($(this.frame),config.duration,offset);
  }
...
Scenes.prototype.getCrtScene = function(){
  return this.scenes[this.scene.current];
}
...
Scenes.prototype.status = function(){
  ...
  this.sceneStatus.html(
    "scene : " + (currentScene.id-1) + " / " + this.scene.total + "<br>" +
    "frame : " + this.frame.current.toFixed(1) + " / " + this.frame.total + "<br>" +
    "frame of current scene : " + (this.frame.current - currentScene.frame.start).toFixed(1)
  )
}
